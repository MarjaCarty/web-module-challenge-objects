...
const mocha = createMenuItem("mocha", 5, "Drinks");
const wrap = createMenuItem("Turkey wrap", 14, "Lunch");
const cake = createMenuItem("Chocolate cake", 10, "Desserts");

console.log(mocha, wrap, cake);
...
reviews.push({name: "Becky", rating: 2, feedback: "Eh, not my type of place. I'll be going elsewhere."});
...
*/
 ...
 function getReviewByRating(arr, reviewNumber) {
    let newReviews = [];
  
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].rating >= reviewNumber && arr[i].rating <= reviewNumber + .99) {
        newReviews.push(arr[i]);
      }
      }
      return newReviews;
    }
